...
class SearchBloc extends Bloc<SearchEvent, SearchState> {
  ...
  changeVarientFunction(
      ChangeVarientItemEvent event, Emitter<SearchState> emit) {
    emit(SearchLoadingState());
    emit(VarientChangedState(
  ...
  addToCartApifunction(
      ItemAddToCartApiEvent event, Emitter<SearchState> emit) async {
    emit(SearchLoadingState());
  ...
  onRemovebuttonclicked(
      RemoveButtonClickedEvent event, Emitter<SearchState> emit) {
    emit(SearchLoadingState());
    emit(RemoveButtonClickedState(type: event.type, index: event.index));
  }

  removeItemsCartApifunction(
      ItemRemoveFromApiEvent event, Emitter<SearchState> emit) async {
    emit(SearchLoadingState());
